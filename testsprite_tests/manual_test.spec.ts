...
test('Homepage loads and Search works', async ({ page }) => {
    ...
    await expect(page.getByRole('heading', { level: 1, name: /results/i }).or(page.getByText(/results/i))).toBeVisible();
});
...
test('Sell Page Loads', async ({ page }) => {
    ...
    await page.goto('http://localhost:3001/sell', { waitUntil: 'domcontentloaded' });
