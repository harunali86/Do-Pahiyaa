...
## 5. API Surface & Architecture Strategy

> **Architecture Strategy Update (2026-02-25)**: 
> The explicitly defined REST endpoints below represent logical business boundaries. However, the implementation architecture is segmented as follows:
> - **Web Platform (Hybrid Approach)**: 
>   - **Internal UI Logic**: Implemented natively via **Next.js 14+ Server Actions** directly bound to React Components for maximum UI speed and type-safety.
>   - **External/Integration Logic**: We explicitly retain **Next.js `/api/v1/...` Route Handlers** for scenarios requiring external access, such as Payment Webhooks (Razorpay), communication webhooks (WhatsApp/Meta), and specific public endpoints.
> - **Future Android & iOS Apps**: Will bypass traditional REST API development entirely by leveraging **Supabase Native SDKs and `pg_graphql`**. Security is handled exclusively via database Row Level Security (RLS) and RPCs, meaning the database is 100% mobile-ready without maintaining a custom Node/Next API layer.

## 5.1 Admin Logic Boundaries (Implemented via RPCs, Server Actions & Route Handlers)
