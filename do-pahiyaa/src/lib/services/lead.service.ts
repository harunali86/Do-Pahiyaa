...
export class LeadService {
    ...
    static async createInquiry(buyerId: string, listingId: string, message: string) {
        ...
        // 1. Enforce ₹49 Monetization Logic: Check for Payment in buyer_unlocks
        ...
        if (unlockError) {
            console.error("[LeadService] Unlock check failed:", unlockError);
            throw new Error("System error verifying payment status.");
        }

        if (!unlock) {
            throw new Error("PAYMENT_REQUIRED: You must unlock this contact for ₹49 first.");
        }

        // 2. Single insert instead of check-then-insert (eliminates race condition)
