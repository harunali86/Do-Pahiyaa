    ...
    try {
        ...
        if (event.event === "payment.captured" || event.event === "order.paid") {
            ...
            if (dealerId && creditsToAdd > 0) {
                // 3. Record Transaction (if not exists)
                if (!existingTxn) {
                ...
                if (dealer) {
                    const { error: updateError } = await admin
                        .from("dealers")
                        .update({
                            credits_balance: dealer.credits_balance + creditsToAdd
                        })
                        .eq("profile_id", dealerId);

                    if (updateError) {
                        console.error("CRITICAL: Credits update failed via webhook", updateError);
                        return NextResponse.json({ error: "Credit Update Failed" }, { status: 500 });
                    }
                    console.log(`[Webhook] Added ${creditsToAdd} credits to dealer ${dealerId}`);
                }
            }
        }
