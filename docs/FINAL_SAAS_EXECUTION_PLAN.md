...
## 2. Auth-Later Development Mode (Temporary)

Use auth bypass only for development speed, without weakening final architecture.

### 2.1 Required environment flags

- `AUTH_MODE=dev_bypass|enforced`
- `DEV_ACTOR_ROLE=buyer|seller|dealer|admin|super_admin`
- `DEV_ACTOR_ID=<uuid>`

### 2.2 Rules

- All API/service authorization checks must remain active.
- In `dev_bypass`, identity source is simulated actor.
- In `enforced`, identity source is Supabase Auth JWT.
- Production deploy must fail if `AUTH_MODE=dev_bypass`.

### 2.3 Enforcement points

1. Middleware route gate
2. API route guard
3. Service-level policy guard
4. UI feature visibility guard
